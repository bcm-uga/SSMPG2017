...
3. When hierarchical population structure (e.g. population divergence with 3 or more pops, range expansion), it generates less false discoveries than $F_{ST}$ statistics.
...
library(bigstatsr)
library(bigsnpr)
rr<-readRDS("sim1a.rds")
G<-add_code256(big_copy(t(rr$G),type="raw"),code=bigsnpr:::CODE_012)
```

```{r,results="hide",warning=FALSE,message=FALSE,echo=FALSE,fig.height = 3, fig.width = 5}
newpc<-snp_autoSVD(G=G,infos.chr =rr$chromosome,infos.pos = rr$position)
plot(newpc)
```
